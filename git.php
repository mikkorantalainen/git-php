  ...
  '.php'     => 'php'  
    ...
    if (isset($_GET['dl']))
        if ($_GET['dl'] == 'targz') 
            ...
            write_plain();
        else if ($_GET['dl'] == 'icon_folder')
            write_icon_folder();
        else if ($_GET['dl'] == 'icon_plain')
            write_icon_plain();
        else if ($_GET['dl'] == 'icon_color')
            write_icon_colored();
...
function git_parse($repo, $what ){
	...
	function write_icon_folder() {
		$img = "\x89\x50\x4e\x47\x0d\x0a\x1a\x0a\x00\x00\x00\x0d\x49\x48\x44\x52".
        ...
        header("Content-Disposition: attachment; filename=icon_folder.png;" );
        header("Expires: +1d");
        echo $img;
        die();
	}
	
	function write_icon_plain() {
		$img =  "\x89\x50\x4e\x47\x0d\x0a\x1a\x0a\x00\x00\x00\x0d\x49\x48\x44\x52" .
		"\x00\x00\x00\x10\x00\x00\x00\x10\x08\x06\x00\x00\x00\x1f\xf3\xff" .
		"\x61\x00\x00\x00\x06\x62\x4b\x47\x44\x00\xff\x00\xff\x00\xff\xa0" .
		"\xbd\xa7\x93\x00\x00\x00\x09\x70\x48\x59\x73\x00\x00\x0b\x13\x00" .
        ...
        header("Content-Type: img/png");
        header("Content-Length: " . $filesize);
        header("Content-Disposition: attachment; filename=icon_plain.png;" );
        header("Expires: +1d");
        echo $img;
        die();
	}
	
	function write_icon_colored() {
		$img = "\x89\x50\x4e\x47\x0d\x0a\x1a\x0a\x00\x00\x00\x0d\x49\x48\x44\x52" .
		"\x00\x00\x00\x10\x00\x00\x00\x10\x08\x06\x00\x00\x00\x1f\xf3\xff" .
		"\x61\x00\x00\x00\x06\x62\x4b\x47\x44\x00\xff\x00\xff\x00\xff\xa0" .
		"\xbd\xa7\x93\x00\x00\x00\x09\x70\x48\x59\x73\x00\x00\x0b\x13\x00" .
		"\x00\x0b\x13\x01\x00\x9a\x9c\x18\x00\x00\x00\x07\x74\x49\x4d\x45" .
		"\x07\xd7\x0a\x07\x13\x35\x33\x17\x09\x18\xa6\x00\x00\x00\x1d\x74" .
		"\x45\x58\x74\x43\x6f\x6d\x6d\x65\x6e\x74\x00\x43\x72\x65\x61\x74" .
		"\x65\x64\x20\x77\x69\x74\x68\x20\x54\x68\x65\x20\x47\x49\x4d\x50" .
		"\xef\x64\x25\x6e\x00\x00\x00\xa8\x49\x44\x41\x54\x38\xcb\x63\x60" .
		"\xa0\x16\x70\x74\x70\xf8\xcf\xc0\xc0\x80\x13\x43\xe5\x31\x00\x23" .
		"\x12\xfb\xff\xdf\xe7\xb3\x70\x5a\xc0\x2c\x99\x86\x4d\x0f\x03\x13" .
		"\xba\x42\x71\x86\x23\x0c\xe2\x0c\x47\x18\x18\x18\x18\x18\x5c\x22" .
		"\x97\x31\xb8\x44\x2e\x43\x76\x25\x03\xd4\x45\x70\xc0\x82\x6e\xc0" .
		"\x4b\x06\x1b\x38\x7b\xcf\xf2\x28\x14\xcd\xbe\x7e\x7e\x70\xd7\xee" .
		"\x3f\x70\x80\x11\xab\x01\x2e\x91\xcb\x18\x96\x5d\x79\x0b\xe7\x8b" .
		"\x5d\xce\x85\xb3\x37\x6f\xda\xc4\xb0\xff\xc0\x01\xf2\xc2\x00\x4b" .
		"\x78\x30\x62\x0d\x03\x64\x7f\xe3\x0b\x0f\xaa\xb9\x80\xa8\x30\x40" .
		"\xb6\x19\x39\x60\x69\x13\x06\x30\x7f\x8b\x33\x1c\x61\x78\xa5\x3b" .
		"\x19\xee\x2a\x64\x9a\xb6\x2e\xa0\x59\x66\x22\x26\x63\x91\x05\x00" .
		"\x0b\x78\x58\xa9\x2d\xf5\x2d\x21\x00\x00\x00\x00\x49\x45\x4e\x44" .
		"\xae\x42\x60\x82";
        $filesize = strlen($img);
        header("Pragma: public"); // required
        header("Expires: 0");
        ...
        header("Content-Type: img/png");
        header("Content-Length: " . $filesize);
        header("Content-Disposition: attachment; filename=icon_color.png;" );
        header("Expires: +1d");
        echo $img;
        die();
	}
